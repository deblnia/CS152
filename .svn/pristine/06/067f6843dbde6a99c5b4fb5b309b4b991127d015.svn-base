...
unsigned int get_guess_feedback(unsigned int guess, unsigned int solution)
{
  ...
  if (extract_digit(guess, 0) > 6 || extract_digit(guess, 1) > 6 || 
      ...
      extract_digit(guess, 2) < 1|| extract_digit(guess, 3) < 1 ) {
      	printf("Each digit of your guess must be between 1 and 6. The number you put in was %d\n", guess);
    return 100;
	}
  ...
  printf("get_guess_feedback - guess=%d, solution=%d, exactMatch=%d, colorMatch=%d, feedback=%d\n", guess, solution, exactMatch, colorMatch, feedback);
  return feedback;
}
